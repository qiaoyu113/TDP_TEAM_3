...
class FootballRobot(ABC):
  ...
  def displayRobotInfo(self) -> None:
    print("Hello! This is robot ", self.name)

  def fetchRobotCoordinates(self) -> list:
  ...
  def obtainRollPitchYawAngles(self) -> list:
  ...
  def checkNewBallDataPresence(self) -> bool:
  ...
  def retrieveSupervisorInformation(self) -> None:
    """Get the latest supervisor data.
    """
    message = self.receiver.getString()
    self.supervisorData = struct.unpack('dd9cc24d', bytes(message, encoding='latin-1'))
    self.receiver.nextPacket()

  def fetchBallPosition(self) -> list:
  ...
  def readLeftUltrasoundDistance(self) -> float:
  ...
  def readRightUltrasoundDistance(self) -> float:
