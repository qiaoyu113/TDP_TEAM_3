...
from Basics.FootballRobotCore import FootballRobot
from Utils.Consts import (TIME_STEP, Motions)
from Utils import Functions
import RedTeamStrategies

class DefenderLeft (FootballRobot):
  def run(self):

    while self.robot.step(TIME_STEP) != -1:

      if self.checkNewBallDataPresence():

        # Do not remove this!
        # ----------------------
        self.retrieveSupervisorInformation()
        # ----------------------

        # Use the ballData (location) to do something.
        ballCoordinate = self.fetchBallPosition()
        # print("RedDefenderLeft - ballCoordinate: ", ballCoordinate)
        selfCoordinate = self.fetchRobotCoordinates()
  ...
  def decideMotion(self, ballCoordinate, selfCoordinate):
    ...
    # Fall Detection
    robotHeightFromGround = self.fetchRobotCoordinates()[2]
    if robotHeightFromGround < 0.2:
      if self.readLeftUltrasoundDistance() == 2.55 and self.readRightUltrasoundDistance() == 2.55:
    ...
    leftDistance = self.readLeftUltrasoundDistance()
    rightDistance = self.readRightUltrasoundDistance()
    
    robotHeadingAngle = self.obtainRollPitchYawAngles()[2]

    # If the ball on the opponent field.
    if RedTeamStrategies.getZone(ballCoordinate) > 9:

      # If the ball on team member.
      if self.calculateBallOwnership()[0] == "R" and self.calculateBallOwnership() != "RED_DEF_L":
      ...
      elif self.calculateBallOwnership()[0] == "B":
    ...
    # The ball on left or middle zone field.
      ...
      # If the ball on team member.
      if self.calculateBallOwnership()[0] == "R" and self.calculateBallOwnership() != "RED_DEF_L" and self.calculateBallOwnership() != "RED_GK":
    ...
    # The ball on right zone.
      ...
      # If the ball on team member.
      if self.calculateBallOwnership()[0] == "R":
