...
class DefenderRight (FootballRobot):
  def run(self):
  ...
  def decideMotion(self, ballCoordinate, selfCoordinate):
    ...
    if RedTeamStrategies.getZone(ballCoordinate) > 9:
      ...
      if self.calculateBallOwnership()[0] == "R" and self.calculateBallOwnership() != "RED_DEF_R":
    ...
    # The ball on right or middle zone field.
      ...
      # If the ball on team member.
      if self.calculateBallOwnership()[0] == "R" and self.calculateBallOwnership() != "RED_DEF_R" and self.calculateBallOwnership() != "RED_GK":
